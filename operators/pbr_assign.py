...
class PBR_OT_AssignTexture(Operator):
    ...
    def execute(self, context):
        ...
        if not obj or not obj.active_material:
            ...
            return {'CANCELLED'}
        if not self.assign_texture_to_input(obj.active_material, self.input_name, self.filepath, self.colorspace):
    ...
    def assign_texture_to_input(material, input_name, image_path, colorspace='sRGB'):
        ...
        # Remove existing chain on this socket
        def gather(n, out):
            if n in out:
                return
            out.add(n)
        ...
        if input_name == 'Normal':
            ...
            # clamp default into [0,1]
            val = principled.inputs[input_name].default_value
            math.inputs[1].default_value = max(0.0, min(val, 1.0))
