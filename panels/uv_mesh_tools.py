...
class REXTools3MeshUVPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        show_seams_section = (
            not prefs or 
            prefs.enable_tool_uv_mesh_area_seam or 
            prefs.enable_tool_uv_mesh_angle_loop_seam or 
            prefs.enable_tool_uv_mesh_seam_island_sharp
        )
        if show_seams_section:
            box = layout.box()
            box.label(text="Seam", icon='STRANDS')
            
            if not prefs or prefs.enable_tool_uv_mesh_area_seam:
                box2 = box.box()
            ...
            if not prefs or prefs.enable_tool_uv_mesh_angle_loop_seam:
                box3 = box.box()
            ...
            if not prefs or prefs.enable_tool_uv_mesh_seam_island_sharp:
                box4 = box.box()
        ...
        if not prefs or prefs.enable_tool_uv_mesh_area_seam_angle:
            layout.operator("rextools3.uv_seam_area_by_angle_modal", text="Area Seam by angle")
        
        if not prefs or prefs.enable_tool_uv_mesh_unwrap_quad:
            box = layout.box()
